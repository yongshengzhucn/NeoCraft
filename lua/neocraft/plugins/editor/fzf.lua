---@class FzfLuaOpts: neocraft.util.pick.Opts
---@field cmd string?
...
local function symbols_filter(entry, ctx)
	if ctx.symbols_filter == nil then
		ctx.symbols_filter = NeoCraft.config.get_kind_filter(ctx.bufnr) or false
	end
	...
	desc = "Awesome picker for FZF (alternative to Telescope)",
	{
		...
		opts = function(_, opts)
			...
			config.defaults.actions.files["ctrl-r"] = function(_, ctx)
				...
				NeoCraft.pick.open(ctx.__INFO.cmd, o)
			end
			...
			return {
				...
				-- Custom NeoCraft option to configure vim.ui.select
					...
					return vim.tbl_deep_extend("force", fzf_opts, {
						...
						winopts = {
							...
							preview = not vim.tbl_isempty(
										NeoCraft.lsp.get_clients({ bufnr = 0, name = "vtsls" })
									)
									and {
										layout = "vertical",
		...
		init = function()
			NeoCraft.on_very_lazy(function()
				vim.ui.select = function(...)
					require("lazy").load({ plugins = { "fzf-lua" } })
					local opts = NeoCraft.opts("fzf-lua") or {}
		...
		keys = {
			...
			-- { "<leader>fR", NeoCraft.pick("oldfiles", { cwd = vim.uv.cwd() }), desc = "Recent (cwd)" },
			-- git
			...
			{ "<leader>sw", NeoCraft.pick("grep_cword"), desc = "Word (Root Dir)" },
			{ "<leader>sW", NeoCraft.pick("grep_cword", { root = false }), desc = "Word (cwd)" },
			{ "<leader>sw", NeoCraft.pick("grep_visual"), mode = "v", desc = "Selection (Root Dir)" },
			{ "<leader>sW", NeoCraft.pick("grep_visual", { root = false }), mode = "v", desc = "Selection (cwd)" },
			-- { "<leader>uC", NeoCraft.pick("colorschemes"), desc = "Colorscheme with Preview" },
			{
				"<leader>ss",
				function()
