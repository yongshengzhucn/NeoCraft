...
return {
  ...
  -- directory is a git repo.
  {
            ...
            if not ok then
              ...
              if not vim.uv.fs_stat(lib) then
                ...
                require("lazy").build({ plugins = { plugin }, show = false }):wait(function()
                  NeoCraft.info("Rebuilding `telescope-fzf-native.nvim` done.\nPlease restart Neovim.")
                end)
              else
    ...
    keys = {
      ...
      --find
      { "<leader>tC", NeoCraft.pick.config_files(), desc = "Find Config File" },
      { "<leader>tf", NeoCraft.pick("files"), desc = "Find Files (Root Dir)" },
      { "<leader>tF", NeoCraft.pick("files", { root = false }), desc = "Find Files (cwd)" },
      ...
      { "<leader>tw", NeoCraft.pick("grep_string", { word_match = "-w" }), desc = "Word (Root Dir)" },
      { "<leader>tW", NeoCraft.pick("grep_string", { root = false, word_match = "-w" }), desc = "Word (cwd)" },
      { "<leader>tw", NeoCraft.pick("grep_string"), mode = "v", desc = "Selection (Root Dir)" },
      { "<leader>tW", NeoCraft.pick("grep_string", { root = false }), mode = "v", desc = "Selection (cwd)" },
      {
        "<leader>ts",
        function()
