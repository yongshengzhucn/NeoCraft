...
function M.detectors.lsp(buf)
  ...
  local clients = NeoCraft.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  NeoCraft.info(lines, { title = "NeoCraft Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    NeoCraft.root.info()
  end, { desc = "NeoCraft roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("neocraft_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return NeoCraft.is_win() and ret:gsub("/", "\\") or ret
end
